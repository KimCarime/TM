...
public class DAOTrucks extends DAOBase {
    ...
    public void newTruck(DataTruck truck){ // add truck in database
        ...
        mDb.insert(TABLE_NAME_TRUCKS,null,value);
    }
    ...
    public List<String> fetchTruckList(){
        List<String> _truckList=null;
        Cursor c=mDb.rawQuery(TABLE_NAME_TRUCKS,new String[]{""});
        if (c.moveToFirst()) {
            for (c.moveToFirst(); !c.isAfterLast(); c.moveToNext()) {
                // Votre code
                _truckList.add(c.getString(c.getColumnIndex(COLUMN_NAME_REGISTRATION)));
            }
            c.close();
        }
        return _truckList;
